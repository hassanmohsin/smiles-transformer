    ...
    def encode(self, src):
        ...
        if batch_size <= 100:
            ...
            while ed < batch_size:
                st += 100
                ed += 100
                out = np.concatenate([out, self._encode(src[:, st:ed])],
                                     axis=0)
            return out
...
def parse_arguments():
    ...
    parser.add_argument("--seq_len",
                        type=int,
                        default=220,
                        help="maximum length of the paired seqence")
...
def main():
    ...
    dataset = Seq2seqDataset(
        pd.read_csv(args.data)["canonical_smiles"].values, vocab)
    test_size = 10000
    ...
    model = TrfmSeq2seq(len(vocab), args.hidden, len(vocab),
                        args.n_layer).cuda()
    ...
    for e in range(1, args.n_epoch):
        for b, sm in tqdm(enumerate(train_loader)):
            ...
            optimizer.step()
            if b % 1000 == 0:
                print("Train {:3d}: iter {:5d} | loss {:.3f} | ppl {:.3f}".
                      format(e, b, loss.item(), math.exp(loss.item())))
            if b % 10000 == 0:
                loss = evaluate(model, test_loader, vocab)
                print(
                    "Val {:3d}: iter {:5d} | loss {:.3f} | ppl {:.3f}".format(
                        e, b, loss, math.exp(loss)))
